...
class LCAUnet(nn.Module):
    ...
    def __init__(self,
        ...
        for i_layer in range(self.num_layers-1):
            layer = AMSFF(in_channels=int(embed_dim * 2 ** (self.num_layers_up-i_layer)),
        ...
        if pretrained_path != "":
            ...
            for k, v in pretrained_dict.items():
                if "downsample." in k:
                    ...
                    current_layer_num = int(k[7:8])
                    current_block_num = int(k[16:17])
                    new_block_k = k[0:16] + str(int(current_block_num + self.depths[current_layer_num] / 2)) + k[17:]
                    # print('current: ',k ,'after: ',new_block_k)
                    # print(current_block_num + self.depths[current_layer_num] / 2)
                    full_dict.update({new_block_k:v})
